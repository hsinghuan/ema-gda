...
class TwoTeachersPerformance:
    def __init__(self, model, src_val_loader, device="cpu"):
        self.device = device
    ...
    @torch.no_grad()
    def _average_confidence(self, model, loader):
        model.eval()
        total_conf = 0
        total_num = 0
        ...
        return total_conf / total_num

    @torch.no_grad()
    def _diff_of_confidence(self, model, loader):
        src_acc = self._oracle_eval_epoch(model, self.src_val_loader)
        src_conf = self._average_confidence(model, self.src_val_loader)
        cur_conf = self._average_confidence(model, loader)
        return src_acc - (src_conf - cur_conf)

    @torch.no_grad()
    def _avg_threshold_confidence(self, model, loader):
        # TODO: get src prob and src labels
        src_probs, src_labels = [], []
        for data, y in self.src_val_loader:
            data = data.to(self.device)
            prob = F.softmax(model(data), dim=1).detach().cpu()
            src_probs.append(prob)
            src_labels.append(y)
        ...
        cur_probs = torch.cat(cur_probs).numpy()

        # TODO: calculate score and pred
        src_scores = get_entropy(src_probs)
        src_preds = np.argmax(src_probs, axis=-1)
        cur_scores = get_entropy(cur_probs)

        # TODO: calculate thres
        _, ATC_thres = find_ATC_threshold(src_scores, src_labels == src_preds)

        # TODO: estimate accuracy
        ATC_accuracy = get_ATC_acc(ATC_thres, cur_scores)
        return ATC_accuracy
