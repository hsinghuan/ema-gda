...
class TwoTeachersAgreement:
    ...
    def _adapt_train_epoch(self, model, train_loader, optimizer, alpha):
        ...
        for data, _ in train_loader:
            ...
            agreement_mask = torch.eq(torch.argmax(cur_teacher_logits, dim=1), torch.argmax(src_teacher_logits, dim=1))
            loss, mask, _ = self._pseudo_label_loss(student_logits[agreement_mask], cur_teacher_logits[agreement_mask], alpha)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    def _calc_alpha(self, loader, confidence_q):
        ...
        for data, _ in loader:
            ...
            src_teacher_logits = self.src_teacher(data)
            agreement_mask = torch.eq(torch.argmax(cur_teacher_logits, dim=1), torch.argmax(src_teacher_logits, dim=1))
            total_agree += agreement_mask.sum().item()
            total_num += len(data)
            prob = torch.softmax(cur_teacher_logits[agreement_mask], dim=1)
        ...
        print("Agreement rate:", total_agree / total_num)
        return alpha
