...
class HierarchicalTeacher:
    ...
    def _adapt_train_eval(self, train_loader, confidence_q, slope, lamb, args, val_loader=None):
        model = deepcopy(self.model)
        inter_teacher = deepcopy(self.inter_teacher)
        self._update_inter_teacher(inter_teacher, model, train_loader, slope)
    ...
    def _update_inter_teacher(self, inter_teacher, model, loader, slope):
        inter_teacher.eval()
        model.eval()
        ...
        print(f"Inter Teacher Entropy: {teacher_ent} Current Model Entropy: {model_ent}: Momentum: {momentum}")
    ...
    def adapt(self, domain_name, train_loader, confidence_q_list, slope_list, lambda_list, args, val_loader=None):
        # pseudo label train loader, val loader
        performance_dict = dict()
        for confidence_q in confidence_q_list:
            for slope in slope_list:
                for lamb in lambda_list:
                    run_name = f"{args.method}_{confidence_q}_{slope}_{lamb}_{args.random_seed}"
                    self.writer = SummaryWriter(os.path.join(args.log_dir, args.dataset, domain_name, run_name))
                    model, val_score, inter_teacher = self._adapt_train_eval(train_loader, confidence_q, slope, lamb, args, val_loader)
                    performance_dict[(confidence_q, slope, lamb)] = {"model": model, "score": val_score, "inter_teacher": inter_teacher}

        best_val_score = -np.inf
        best_confidence_q, best_slope, best_lamb = None, None, None
        best_model, best_inter_teacher = None, None
        for (confidence_q, slope, lamb), ckpt_dict in performance_dict.items():
            print(f"Confidence Q: {confidence_q} Slope: {slope} Lambda: {lamb} Score:", ckpt_dict["score"])
        ...
        print(f"Best Confidence Q: {best_confidence_q} Best Slope: {best_slope} Best Lamb: {best_lamb}")
        ...
        if val_loader:
            ...
            run_name = f"{args.method}_all_{best_confidence_q}_{best_slope}_{best_lamb}_{args.random_seed}"
            self.writer = SummaryWriter(os.path.join(args.log_dir, args.dataset, domain_name, run_name))
            best_model, _, best_inter_teacher = self._adapt_train_eval(loader, best_confidence_q, best_slope, best_lamb, args)
