...
class EntropyPLinearCalibratedEnsemble:
    def __init__(self, model, device="cpu"):
        self.device = device
        self.model = model.to(self.device)
        self.src_teacher = ModelWithTemperature(deepcopy(model)).to(self.device)
    ...
    def _adapt_train_eval(self, train_loader, confidence_q, slope, args, val_loader=None):
        src_weight = self._calc_src_weight(train_loader, slope)
        ...
        for e in range(1, args.adapt_epochs + 1):
            ...
            if not val_loader:
                score = train_score
                print(f"Confidence q: {confidence_q} Slope: {slope} Epoch: {e} Train Loss: {round(train_loss, 3)} Train Acc: {round(train_acc, 3)}")
                continue
            ...
            score = val_score
            print(f"Confidence q: {confidence_q} Slope: {slope} Epoch: {e} Train Loss: {round(train_loss, 3)} Train Acc: {round(train_acc, 3)} Val Loss: {round(val_loss, 3)} Val Acc: {round(val_acc, 3)}")
    ...
    @torch.no_grad()
    def _calc_src_weight(self, loader, slope):
        ...
        src_weight = torch.clip(0.5 + slope * (cur_teacher_entropy - src_teacher_entropy), 0, 1)
    ...
    @torch.no_grad()
    def _pseudo_label_val_loader(self, val_loader):
        self.model.eval()
        data_all, pl_all = [], []
        for data, _ in val_loader:
            data = data.to(self.device)
            pl = torch.argmax(self.model(data), dim=1)
            data_all.append(data)
            pl_all.append(pl)
        data_all = torch.cat(data_all)
        pl_all = torch.cat(pl_all)
        dataset = TensorDataset(data_all, pl_all)
        pl_val_loader = DataLoader(dataset, batch_size=256, shuffle=True)
        return pl_val_loader


    def adapt(self, domain_name, train_loader, confidence_q_list, slope_list, args, val_loader=None):
        # TODO: calibrate source teacher w/ pseudo-labels given by current model on val loader
        if val_loader:
            pl_val_loader = self._pseudo_label_val_loader(val_loader)
            self.src_teacher.set_temperature(pl_val_loader)
        performance_dict = dict()
        for confidence_q in confidence_q_list:
            for slope in slope_list:
                run_name = f"{args.method}_{confidence_q}_{slope}_{args.random_seed}"
                self.writer = SummaryWriter(os.path.join(args.log_dir, args.dataset, domain_name, run_name))
                model, score = self._adapt_train_eval(train_loader, confidence_q, slope, args, val_loader)
                performance_dict[(confidence_q, slope)] = {"model": model, "score": score}
        ...
        best_slope = None
        best_model = None
        for (confidence_q, slope), ckpt_dict in performance_dict.items():
            ...
            if score > best_score:
                ...
                best_slope = slope
                best_score = score
            print(f"Confidence Q: {confidence_q} Slope: {slope} Score: {score}")

        print(f"Best Confidence Q: {best_confidence_q} Best Slope: {best_slope}")
        ...
        if val_loader:
            ...
            run_name = f"{args.method}_all_{best_confidence_q}_{best_slope}_{args.random_seed}"
            self.writer = SummaryWriter(os.path.join(args.log_dir, args.dataset, domain_name, run_name))
            best_model, _ = self._adapt_train_eval(loader, best_confidence_q, best_slope, args)
